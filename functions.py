# -*- coding: utf-8 -*-
"""
Created on Sun Oct 02 12:17:25 2021

@author: Yauheny_Shaludzko
"""

# HW2 Refactor using functional approach
...
def dict_list_creation():
    ...
    for i in range(r.randint(2, 10)):  # loop is used to fill list with random number of dictionaries
        ...
        d_list.append(random_dict)  # generated dictionary is added into dict_list
    return d_list


dict_list = dict_list_creation()
print("dict_list:", dict_list)  # print output result


def index_return(key1):  # creating func which returns index of dict, where key from parameter with max value
...
# HW3 Refactor using functional approach

def normal_text(text):
    """Function which refactor given text according to homework 3 tasks"""
    sentences = text.split(".")  # split text to sentences on dots
    sentfixed = [i.strip(' \t\n\r').capitalize() for i in sentences]  # trim whitespaces, capitalize sentences via gen
