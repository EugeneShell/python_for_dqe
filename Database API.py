...
import SQLmodule  # importing SQL module to add DB functionality
...
def start():  # main start function implementing
    ...
    if ch == 1:
        ...
        while choice != 4:  # exit from app
            ...
            post = None
            dbcon = DBcon()  # creating DBcon instance

            if choice == 1:
                post = News()  # news posting
                dbcon.insert_news(post.date, post.city, post.text)  # inserting news into news table
            elif choice == 2:
                post = PrivateAd()  # private ads posting
                dbcon.insert_ads(post.date, post.text, post.date_end)  # inserting ads into privatead table
            elif choice == 3:
                post = Obituary()  # obituary posting
                dbcon.insert_obituary(post.date, post.city, post.burial_date,
                                      post.burial_place)  # inserting obituaries into obituary table
            else:
            ...
            dbcon.close_all()  # closing db connections
